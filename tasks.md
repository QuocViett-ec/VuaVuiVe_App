...
- [ ] Search → filter products + auto scroll
- [ ] Shortcuts (Flash Sale, Bánh, Mì, Bia, Sữa) → filter category
...
- [ ] Nút "+" thêm vào cart + Toast
- [ ] Promo popup hiện 1 lần duy nhất
...
- [ ] Swagger UI mở được tại `http://localhost:3000/swagger-ui.html`
- [ ] `POST /api/auth/register` → tạo user thành công
- [ ] `POST /api/auth/login` → nhận đúng `accessToken` + `refreshToken`
- [ ] `GET /api/products` → trả về 92 sản phẩm
- [ ] `GET /api/categories` → trả về 8 danh mục
- [ ] `POST /api/orders` (CUSTOMER token) → tạo đơn thành công
- [ ] `GET /api/orders/my` → đơn của user đó
- [ ] `PATCH /api/orders/{id}/cancel` → hủy đơn
- [ ] `GET /api/orders/shipper` (SHIPPER token) → đơn của shipper
- [ ] `GET /api/recipes` → 41 công thức
